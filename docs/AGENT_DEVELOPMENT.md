...
| LLM 服务 (重构) | ✅ 完成 | `src/services/llm/` (7个提供商) |
...
- ✅ 多模式系统 (editor/organizer/researcher/writer)
- ✅ LLM 服务重构 (统一 7 个提供商)
- ✅ 编辑器简化 (移除 TipTap，统一使用 CodeMirror)
- [ ] `organize` 工具
- ⏳ 流式响应 (Moonshot/DeepSeek 已完成，UI 接入，需要检查不同的支持是否全部完成)
- ✅ 历史记录持久化 (会话管理 + 本地存储)
- [ ] Agent 编辑实时预览 (聚焦文件 + 实时显示修改)
...
src/services/llm/             # ← 统一 LLM 服务 (重构)
├── types.ts                  # 类型 + PROVIDER_REGISTRY
├── config.ts                 # 配置管理
├── index.ts                  # callLLM 入口
└── providers/                # 7 个提供商
    ├── anthropic.ts          # Claude
    ├── openai.ts             # GPT
    ├── moonshot.ts           # Kimi (支持 thinking)
    ├── deepseek.ts           # DeepSeek (支持 reasoning)
    ├── groq.ts               # 快速推理
    ├── openrouter.ts         # 多模型网关
    └── ollama.ts             # 本地模型

src/services/rag/             # RAG 系统
├── types.ts                  # RAG 类型定义
├── index.ts                  # 模块导出
...
src/editor/                   # ← 编辑器 (简化后)
├── CodeMirrorEditor.tsx      # 主编辑器 (支持 livePreview 开关)
├── Editor.tsx                # 编辑器容器 (三模式切换)
└── ReadingView.tsx           # 只读渲染视图

src-tauri/src/vector_db/      # 向量数据库后端
├── mod.rs                    # 向量数据库核心
...
- [x] **LLM 服务重构** ✅
  - 统一 7 个提供商 (Anthropic/OpenAI/Moonshot/DeepSeek/Groq/OpenRouter/Ollama)
  - PROVIDER_REGISTRY 元数据
  - 支持 thinking/reasoning 模式

- [x] **编辑器简化** ✅
  - 移除 TipTap，统一使用 CodeMirror
  - 三模式：阅读/实时预览/源码
  - livePreview 属性控制语法隐藏
...
- ⏳ **流式响应** (部分完成)
  - [x] Moonshot Provider 流式 + thinking
  - [x] DeepSeek Provider 流式 + reasoning
  - [ ] 其他 Provider 流式支持
  - [ ] Agent UI 流式渲染

- [x] **历史记录持久化** ✅
  - 对话保存到本地存储 (Zustand persist)
  - 跨会话恢复对话历史
  - 对话列表管理（新建/删除/切换/重命名）
  - 自动生成会话标题

- [ ] **Agent 编辑实时预览**
  - 自动聚焦：Agent 执行 edit_note 时，主编辑器自动切换到目标文件
  - 实时显示：用户能看到修改过程（类似看着有人在编辑）
  - 差异高亮：标记出被改动的行/内容
  - 参考：Cursor AI 编辑的实时预览效果

### Phase 4: 优化与扩展 (待开发)

- [ ] **长期记忆系统 (mem0 式)**
  - 自动提取对话中的关键信息（用户偏好、项目上下文、常用操作）
  - 记忆向量化存储 + 语义检索
  - 跨对话持久化，AI 能记住用户习惯
  - 记忆管理 UI（查看/编辑/删除记忆）
...
export interface AgentConfig {
  // AI 提供商配置 (支持 7 个提供商)
  ai: {
    provider: "anthropic" | "openai" | "moonshot" | "deepseek" | "groq" | "openrouter" | "ollama";
    apiKey: string;
    model: string;
    baseUrl?: string;
  };
